...
cd('E:\anand_animation\')

treats={'blank','leftward'};
d2=1; %2d data only
...
for tt=1:length(treats)
  if save_movie
    v=VideoWriter(['trial' treats{tt} '.avi'],'uncompressed avi');
...
%     v.Quality=90;
  ...
  A=importdata([treats{tt} '_platformxyzpts.csv']);
  platform1=A.data(1,1:3);
  platform2=A.data(1,4:6);
  
  aud_fn=dir(['rousettus_*_' treats{tt} '_mic_data_detect.mat']);
  load(aud_fn.name);
...
%       for mm=1:length(goodch)
      if d2
        pm=plot(bat(call_fr,1)+norm_I_mic_vec(:,1),...
          bat(call_fr,2)+norm_I_mic_vec(:,2),'.-','color',[.4 .4 .4]);
      else
...
%       pf=[];
    ...
    if save_movie
      ...
      audiowrite([treats{tt} '.wav'],y./max(y)/2,fs/(track.fs*10/vid_frate))
    end
end